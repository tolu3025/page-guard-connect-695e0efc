-- Migration: When all grades for a matric_no are deleted, reset related submissions
-- and clean up the cgpa_summary so the Overview page stays in sync.

-- 1. Recreate the recalculate trigger to also handle cgpa_summary cleanup
...
BEGIN
  ...
  -- Count remaining grades for this student
  SELECT COUNT(*) INTO v_grade_count
  FROM public.grades
  WHERE matric_no = v_matric;

  -- If no grades left, delete cgpa_summary row entirely and revert submissions
  IF v_grade_count = 0 THEN
    DELETE FROM public.cgpa_summary WHERE matric_no = v_matric;

    -- Reset any APPROVED submissions back to PENDING so student can re-submit
    UPDATE public.result_submissions
    SET status = 'PENDING'
    WHERE matric_no = v_matric AND status = 'APPROVED';
  ...
  -- Upsert into cgpa_summary
  INSERT INTO public.cgpa_summary (
    matric_no, student_name, level,
    total_credit_units, total_weighted_points,
    cgpa, classification, status, last_updated
  ) VALUES (
    v_matric, v_student_name, v_level,
    v_total_cu, v_total_wp,
    v_cgpa, v_classification, v_status, NOW()
  )
